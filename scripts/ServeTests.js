...
export const CreateTestServer = (directory = projectDirectory) =>
  http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (["/", "/notes", "/dom", "/utilities", "/components"].some((route) => requestPath === showcase + route) || /^\/tests\/browser\/showcase\/utilities\/[A-Za-z0-9_-]+\/?$/.test(requestPath)) {
      ...
      if (
        !IsWithin(root, allowedRoot) ||
        !IsWithin(allowedRoot, file) ||
        path
          .relative(root, file)
          .split(path.sep)
      ...
      if (file === path.join(root, "tests/browser/showcase/index.html")) {
        contents = contents.toString("utf8").replace("<head>", `<head><base href="${showcase}/">`);
      }
