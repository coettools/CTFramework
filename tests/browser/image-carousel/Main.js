...
CT(async () => {
  ...
  try {
    ...
    root.querySelector('[aria-label="Show image 2"]').click();
    await Settle();
    Assert(Count() === "2 of 3" && root.querySelector('[aria-current="true"]').textContent.trim() === "2", "The selector marks the active image.");
    const notes = document.querySelector("#notes");
    notes.focus();
    await app.SetState({ Notes: "Typing across a parent update" });
    await Settle();
    ...
    app.SetState({ Loop: false });
    await Settle();
    root.querySelector('[aria-label="Show image 3"]').click();
    await Settle();
    ...
    app.SetState({ Images: CarouselImages.slice(0, 1) });
    await Settle();
    await Settle();
    ...
    app.SetState({ Images: CarouselImages });
    await Settle();
