...
export class WorkspacePreview extends Component {
  ...
  RenderCounter() {
    return Card({
      ...
      Content: html`<div class="workspace-stack">
        ...
        <div class="workspace-actions">
          <button ${CT.On("click", () => this.ChangeCount(1))}>Increment</button><button class="ct-button-secondary" ${CT.Attr("disabled", this.state.Count === 0)} ${CT.On("click", () => this.ChangeCount(-1))}>Decrement</button>
        </div>
      </div>`,
    });
  }
  ...
  RenderSettings() {
    return Card({
      ...
      Content: html`<div class="workspace-stack">
        ...
        <div class="workspace-actions">
          ${Tooltip({ Text: "Show a local notification", Position: "top", Content: html`<button ${CT.On("click", () => this.SetState({ ToastVisible: true }))}>Save settings</button>` })}<button
            class="ct-button-secondary"
            ${CT.On("click", () => this.SetState({ DialogOpen: true }))}
          >
            Open dialog
          </button>
        </div>
  ...
  Render() {
    return html`<div class="workspace">
      <header class="workspace-header">
        <strong>coettools <span>/ workspace</span></strong
        >${Badge({ Text: "Local example" })}
      </header>
      <div class="workspace-measure">
        <div class="workspace-body">
          <div class="workspace-navigation">
            ${SideNavigation({
              Title: "On this page",
              ActiveId: this.state.Section,
              Items: [
                { Id: "counter", Label: "Counter & settings" },
                { Id: "table", Label: "Data table" },
                { Id: "feedback", Label: "Feedback" },
              ],
              OnNavigate: (item) => this.Navigate(item),
            })}
          </div>
          <div class="workspace-content">
            <header class="workspace-heading">
              <p class="workspace-eyebrow">Raw JavaScript components</p>
              <h3>Tools for the work.</h3>
              <p class="workspace-help">The same controls in every direction. Try the counter, search, and feedback.</p>
            </header>
            <div class="workspace-controls" ${CT.Attr("id", this.GetId("counter"))}>${this.RenderCounter()}${this.RenderSettings()}</div>
            <section ${CT.Attr("id", this.GetId("table"))} aria-label="Components table">
              ${DataTable({
                RowKey: "Name",
                Data: Records,
                PageSize: 3,
                ...
                Columns: [
                  ...
                  { Key: "Status", Title: "Status", Render: (_, value) => Badge({ Text: value, Type: "success" }) },
                ],
              })}
            </section>
            <section class="workspace-feedback" ${CT.Attr("id", this.GetId("feedback"))} aria-label="Feedback examples">
              <div class="workspace-statuses">
                ${Badge({ Text: "Ready", Type: "success" })}${Badge({ Text: "Pending", Type: "warning" })}${Badge({ Text: "Failed", Type: "danger" })}<button
                  class="ct-button-secondary"
                  ${CT.On("click", () => this.SetState({ PopupOpen: true }))}
                >
                  Open popup
                </button>
              </div>
      ...
      ${Dialog({
        ...
        Content: html`<p>Check the backdrop, panel, and button colors. No project files will change.</p>`,
        OnClose: () => this.SetState({ DialogOpen: false }),
        Actions: [{ Label: "Back to preview", OnClick: () => this.SetState({ DialogOpen: false }) }],
      })}
