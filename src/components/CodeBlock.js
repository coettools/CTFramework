...
export class CodeBlockComponent extends Component {
  ...
  RenderLine(line, index) {
    const tokens = line.map((token) => (token.Type === "plain" ? token.Text : html`<span ${CT.Attr("className", `ct-code-token-${token.Type}`)}>${token.Text}</span>`));
  ...
  Render() {
    ...
    const message = this.state.CopyErrorCode === code ? "Copy unavailable. Select the code and copy it manually." : this.state.CopiedCode === code ? "Copied" : "";
      ...
      <section ${CT.Attr("className", `ct-code-block${this.state.Wrap ? " is-wrapped" : ""}${LineNumbers ? " has-line-numbers" : ""}`)}>
        <header class="ct-code-toolbar">
          ...
          <div class="ct-code-actions">
            <button type="button" class="ct-button-secondary" aria-label="Wrap lines" ${CT.Attr("aria-pressed", String(this.state.Wrap))} ${CT.On("click", () => this.ToggleWrap())}>Wrap</button>
            ${Copy ? html`<button type="button" class="ct-button-secondary" aria-label="Copy code" ${CT.Attr("disabled", this.state.IsCopying)} ${CT.On("click", () => this.CopyCode())}>Copy</button>` : null}
          </div>
        </header>
        <pre class="ct-code-viewport" tabindex="0" role="region" ${CT.Attr("aria-label", Title || `${LanguageLabels[language]} code`)}><code class="ct-code-source">${this._lines.map((line, index) =>
