...
export class AccordionComponent extends Component {
  ...
  ToggleItem(itemId) {
    ...
    const nextOpenIds = this.props.Multiple ? (isOpen ? openIds.filter((openId) => openId !== itemId) : [...openIds, itemId]) : isOpen ? [] : [itemId];
            ...
            ...html`
              <article class="ct-accordion-item">
                <h3 class="ct-accordion-heading">
                  <button type="button" class="ct-accordion-trigger" ${CT.Attr("id", `${id}-trigger`)} ${CT.Attr("aria-controls", `${id}-panel`)} ${CT.Attr("aria-expanded", String(isOpen))} ${CT.On("click", () => this.ToggleItem(item.Id))}>
                    ${item.Title}
                  </button>
                </h3>
                <div class="ct-accordion-content" ${CT.Attr("role", this.props.Multiple && Items.length > 6 ? "group" : "region")} ${CT.Attr("id", `${id}-panel`)} ${CT.Attr("aria-labelledby", `${id}-trigger`)} ${CT.Attr("hidden", !isOpen)}>
                  ${item.Content}
                </div>
              </article>
            `,
